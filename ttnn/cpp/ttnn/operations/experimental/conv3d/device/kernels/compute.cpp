                    ...
                    for (uint32_t h_block = h_out_start; h_block < h_out_end; h_block += H_block_size) {
                        for (uint32_t w_block = w_out_start; w_block < w_out_end; w_block += W_block_size) {
                            ...
                            if (!is_reducer) {
                                ...
                                // reduction.
                                if constexpr (use_fp32_partials) {
                                    // FPU accumulate path for fp32 reduction (same pattern as
                                    // rmsnorm_pre_allgather_2d): add_tiles with acc_to_dest=true
                                    // accumulates into DST at full fp32 precision.
                                    // DST += tile + 0 for each partial (reducer's own + workers').
                                    // The zero CB is filled by the writer at kernel startup.
                                    cb_wait_front(cb_zero_tiled, 1);
                                    reconfig_data_format(cb_matmul_interm_tiled, cb_zero_tiled);
                                    pack_reconfig_data_format(cb_matmul_interm_tiled);
                                    add_tiles_init(cb_matmul_interm_tiled, cb_zero_tiled, true);
                                }
                                for (uint32_t i = 0; i < num_workers; i++) {
                                    ...
                                    if constexpr (use_fp32_partials) {
                                        for (uint32_t t = 0; t < output_tiles; t++) {
                                            tile_regs_acquire();
                                            // Accumulate reducer's partial: DST[0] += own + 0
                                            add_tiles(cb_matmul_interm_tiled, cb_zero_tiled, 0, 0, 0);
